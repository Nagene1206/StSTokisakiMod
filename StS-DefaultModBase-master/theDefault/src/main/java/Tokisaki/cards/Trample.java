...
public class Trample extends CustomCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        int dmg = (int)(m.maxHealth * 0.1);
        if(!upgraded)
            AbstractDungeon.actionManager.addToBottom(new DamageAction(m, new DamageInfo(p, dmg,damageTypeForTurn),AbstractGameAction.AttackEffect.SMASH));
        else
            AbstractDungeon.actionManager.addToBottom(new DamageAction(m, new DamageInfo(p, dmg*2,damageTypeForTurn),AbstractGameAction.AttackEffect.SMASH));
    }
